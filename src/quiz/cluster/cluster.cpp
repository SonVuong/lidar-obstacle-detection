...
void proximity(const std::vector<std::vector<float>> points,
	std::vector<int>& cluster, std::vector<bool>& processed, int idx, KdTree* tree, float distanceTol) {

		processed[idx] = true;
		cluster.push_back(idx);
		std::vector<int> neighbor_point = tree->search(points[idx], distanceTol);

		for(int neighbor_id: neighbor_point) {
			if (!processed[neighbor_id]) {
				proximity(points, cluster, processed, neighbor_id, tree, distanceTol);
			}
		}
